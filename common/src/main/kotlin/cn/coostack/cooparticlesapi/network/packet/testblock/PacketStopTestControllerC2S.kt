...
@CooAutoRegister
class PacketStopTestControllerC2S() : CooPacket() {
    ...
    override fun id(): ResourceLocation {
        return ResourceLocation.fromNamespaceAndPath(CooParticlesConstants.MOD_ID, "stop_test_controller_c2s")
    }

    override fun onServerReceive(context: ServerContext) {
        val sender = context.sender
        if (!sender.isCreative) return
        TestControllerBlockAccess.find(sender.server, dimension, blockPos)?.stopTest()
    }
}
