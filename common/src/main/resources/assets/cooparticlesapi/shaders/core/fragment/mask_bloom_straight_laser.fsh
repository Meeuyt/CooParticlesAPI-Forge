...
void main() {
    ...
    float smoke = texture(
        impactNoise,
        fract(vec2(angle * 3.4 - textureTime * 0.46, beamCoord.y * 4.7 - textureTime * 0.58))
    ).r;
    float smokeFine = texture(
        impactNoise,
        fract(vec2(angle * 6.2 + textureTime * 0.34, beamCoord.y * 8.4 - textureTime * 0.92))
    ).g;
    ...
    float shock = smoothstep(0.32, 0.88, smoke + abs(impactOffset.x) * 1.15) *
        smoothstep(0.08, 0.92, baseRadial);
    ...
    float body = shell * 0.24 + core * 0.38 + rim * (0.24 + shock * 0.16) + flow * 0.08 +
        wave * 0.10 + shock * 0.12 + smokeRidge * 0.30;
    ...
    float outerBody = rim * (0.28 + shock * 0.14) + smokeRidge * 0.46 + shell * 0.14 +
        flow * 0.055 + wave * 0.06;
    outerBody *= 1.0 - smokeGroove * 0.64;
    float shapeAlpha = saturate(alpha * (outerBody + 0.012 + smokeRidge * 0.06)) *
    ...
    beamColor *= brightness *
        (0.52 + rim * 0.16 + flow * 0.04 + wave * 0.04 + shock * 0.08 + smokeRidge * 0.24);
